...
class MinutesProviderPage extends ChangeNotifier{
  ...
  Future getListOfMinutes(data) async{
    var response = await networkHandler.post1('/get-list-minutes',data);
    if (response.statusCode == 200 || response.statusCode == 201) {
      log.d("get-list-minutes form provider response statusCode == 200");
      var responseData = json.decode(response.body) ;
      var responseMinuteData = responseData['data'];
      minutesData = Minutes.fromJson(responseMinuteData);
      log.d(minutesData!.minutes!.length);
      notifyListeners();
    } else {
      log.d("get-list-minutes form provider response statusCode unknown");
